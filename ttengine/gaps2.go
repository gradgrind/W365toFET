	...
	for _, cl := range ttinfo.Db.Classes {
		...
		for agn, ag := range aglist {
			...
			repeatday:
				...
				a := ttinfo.Activities[aixlast]
				// If the last lesson is a lunch-break, it can be
				// replaced by a "blocker".
				if a.Lesson == nil {
					// Convert the activity to a blocker
					a.PossibleSlots = nil
					a.Fixed = true
					pmon.added[aixlast] = -1
					// If there are gaps, repeat check on this day
					if ngaps != 0 {
						goto repeatday
					}
					continue // Otherwise go to next day
				}
