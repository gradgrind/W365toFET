...
//TODO: This assumes that the "Name" field is to be used as vertical header.
#let vheaders = ()
...
let ttvcell(
    ...
    let texts = (
        SUBJECT: Subject,
          CLASS: Groups.map(it=>it.split(".").at(0)).join(JOINSTR),
          TEACHER: Teachers.join(JOINSTR),
          ROOM: Rooms.join(JOINSTR),
          GROUP: Groups.join(JOINSTR),
          CLASS_WITH_GROUP: Groups.join(JOINSTR),
    )
...
// Count page numbers
let pageno = 1
let pagetotal = int((rows.len() + nprows -1) / nprows)+1
while irow < nrows +1 {
  ...
  let legendItems(ilist) = {
        let items = ()
        for (Shortcut, Explanation) in ilist {
            items.push(box(Shortcut + " = " + Explanation))
        }
        items.join(", ")
    }
  ...
  let subjectsLegend = legend.at("Subjects", default: ())
  let teachersLegend = legend.at("Teachers", default: ())
  let roomsLegend = legend.at("Rooms", default: ())


  let legendBlock = text(10pt)[ Hallo Welt!
                #if legendRemark != "" [*Hinweis:* #legendRemark  ]
                #if footnotesLegend.len() != 0 [*Anmerkungen:* #legendItems(footnotesLegend) ]
                #if subjectsLegend.len() != 0 [*Fächer:* #legendItems(subjectsLegend) ]
                #if teachersLegend.len() != 0 [*Lehrkräfte:* #legendItems(teachersLegend) ]
                #if roomsLegend.len() != 0 [*Räume:* #legendItems(roomsLegend) ]
            ]
...
irow += 1
    ...
    if iy == nprows or irow == nrows{
        ...
        block(height: TITLE_HEIGHT, above: 0mm, below: 4mm, inset: 0mm)[
            #place(top)[= #doctitle]
