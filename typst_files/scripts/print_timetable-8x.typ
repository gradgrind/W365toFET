...
#let fieldPlacements = typstMap.at("FieldPlacement", default: (:))
    ...
    // fallback
    fieldPlacements = base.boxText.at(tableType, default: (:))
}
...
#for p in xdata.Pages {
	...
	let legend = pagesFromTypstMap.at(npage, default: (:)).at("Legend",
        default: legend0)
    npage += 1
    ...
    - PAGE_BORDER.bottom - TITLE_HEIGHT - footerHeight - HEADING_TO_PLAN_DISTANCE - PLAN_TO_FOOTER_DISTANCE - 4mm)
  ...
  let ttcell(
      ...
      let texts = (
          SUBJECT: Subject,
          CLASS: Groups.map(it=>it.split(".").at(0)).join(","),
          TEACHER: Teachers.join(","),
          ROOM: Rooms.join(","),
          GROUP: Groups.join(","),
          CLASS_WITH_GROUP: Groups.join(","),
      )
...
let fgWCAG2(colour) = {
	...
	let fgW365(colour) = {
    let (r,g,b,a) = rgb(colour).components()
    	let l = 0.2126 * calc.pow(r / 100%, 2.2) + 0.7152 * calc.pow(g / 100%, 2.2) + 0.0722 * calc.pow(b/ 100%, 2.2) 
  ...
  let subtitle = typstMap.at("Subtitle", default: "")
    ...
    let lastChange = pagesFromTypstMap.at(npage - 1,default: (:)).at("LastChange", default: "")
    if lastChange == "" {
	...
	// HEADER
    block(height: TITLE_HEIGHT, above: 0mm, below: HEADING_TO_PLAN_DISTANCE, inset: 0mm)[
        #place(top)[= #title]
